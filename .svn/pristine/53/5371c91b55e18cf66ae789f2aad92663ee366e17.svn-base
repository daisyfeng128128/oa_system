...
public class ActoresServiceImpl implements IActoresService {
	...
	@Override
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public NPageResult<?> readPagesActoresOnline(int pageSize, int curPage, String sortName, String sortOrder, String genre, String isleave, SearchFilter filter,
			List<RoleDetailsAtts> platList, List<RoleDetailsAtts> attsList, RoleUsers users, String srealname, String saliasname, String sphone, String fastArg, String snumber) {
		...
		if (operator.getGenrer() == Genres.MANAGER || operator.getGenrer() == Genres.SUPERS || Boolean.valueOf(isViews)) {
			...
			rows = this.actoresDAO.readPagesPlatformsActores(pageSize, curPage, sortName, sortOrder, genre, isleave, filter, platList, operator, users, srealname, saliasname,
					sphone, fastArg, snumber);
		}
		if (rows != null && rows.getData().size() > 0) {
			List<ActoresOnlineBean> listBean = new ArrayList<ActoresOnlineBean>();
			BranchOffice branchOffice = null;
	...
	public ActoresOfflineBean editActoresofflineById(Integer id) {
		if (id != null && id > 0) {
			Actores post = this.actoresDAO.readActores(id);
			Employee emp = this.employeeService.readEmployeebyNumber(post.getNumber(), post.getBranchs());
	...
	public boolean addLastNewActores(Actores post, Integer pushMoney, Integer empid, String beLiveTime, String loadTable, Integer channel, Integer plat,/* 最新开播时间 */
		...
		if (post != null && post.getId() != null && post.getId().intValue() > 0) { // 修改
			...
			if (this.actoresDAO.saveActores(post)) {
				if (empid != null && empid.intValue() > 0) {
					this.monitorLogService.logsUpdate(request, "线下艺人" + post.getRealname() + ",id:" + post.getId() + ",n: " + post.getNumber() + ",b:" + post.getBasicSalary()
							+ ",p:" + post.getPushMoney());
				} else {
					this.monitorLogService.logsUpdate(request,
							"线上艺人" + post.getAliasname() + ",id:" + post.getId() + ",b:" + post.getMinimumGuarantee() + ",p:" + post.getPushMoney());
				}
			...
			if (this.actoresDAO.saveActores(post)) {
				...
				if (post.getGenre() == LineGenres.ONLINE) {
					...
					if (this.platformsActoresService.addNewActores(pact, post.getId(), plat, post.getMinimumGuarantee().toString(), channel, beliveTime, pushMoney, request))
						return true;
				}
				return true;
			}
