		...
		gotoPage : function(b, c) {
			...
			if (a.trim(c) == "" || isNaN(c)) {
				...
				if (parseInt(c) < 1 || parseInt(c) > b.totalPages) {
					a.fn.bsgrid_paging.alert(a.bsgridLanguage.needRange(1,
							b.totalPages))
				} else {
	...
	$.fn.bsgrid = {
		...
		init : function(gridId, settings) {
			...
			if (!options.settings.pageAll) {
				...
				try {
					var minWidth = $.trim($("#" + options.pagingId).children()
							.width());
					minWidth = minWidth == "" ? 0 : parseInt(minWidth);
					if (minWidth != 0) {
						$("#" + gridId).css("min-width", minWidth + 16);
						$("#" + options.pagingOutTabId).css("min-width",
								minWidth + 16)
					}
					$("#" + options.pagingOutTabId).width(
							$("#" + gridId).width());
					$(window).resize(
							function() {
								$("#" + options.pagingOutTabId).width(
			...
			try {
				$.fn.bsgrid.extendInitGrid.initGridExtendOptions(gridId,
						options)
			} catch (e) {
			}
			...
			$.fn.bsgrid
					...
					.each(
							function() {
								...
								var sortInfo = $.trim($(this).attr(
										colsProperties.sortAttr));
								...
								columnModel.index = $.trim($(this).attr(
										colsProperties.indexAttr));
								columnModel.render = $.trim($(this).attr(
										colsProperties.renderAttr));
								columnModel.tip = $.trim($(this).attr(
										colsProperties.tipAttr));
								var maxLen = $.trim($(this).attr(
										colsProperties.lengthAttr));
								columnModel.maxLen = maxLen.length != 0 ? parseInt(maxLen)
										: colsProperties.maxLength;
								var align = $.trim($(this).attr(
										colsProperties.alignAttr));
								columnModel.align = align == "" ? colsProperties.align
										: align;
								columnModel.hidden = $.trim($(this).attr(
										colsProperties.hiddenAttr));
									...
									for ( var key in options.settings.event.customRowEvents) {
										trObj
												.bind(
														...
														function(event) {
															options.settings.event.customRowEvents[key]
																	(
																			event.data.record,
																			event.data.rowIndex,
																			event.data.trObj,
														...
														for ( var key in options.settings.event.customCellEvents) {
															tdObj
																	.bind(
																			...
																			function(
																					event) {
																				options.settings.event.customCellEvents[key]
																						(
																								event.data.record,
																								event.data.rowIndex,
																								event.data.colIndex,
		...
		addRowsClickEvent : function(options) {
			$.fn.bsgrid.getRows(options).filter(
					":lt(" + options.curPageRowsNum + ")").click(
					function() {
						if ($(this).hasClass("selected")) {
							$.fn.bsgrid.unSelectRow(options)
						} else {
							$.fn.bsgrid.selectRow($.fn.bsgrid.getRows(options)
									.index($(this)), options)
						}
					})
		},
		...
		getColCells : function(col, options) {
			return $.fn.bsgrid.getRows(options).find(
					"td:nth-child(" + (col + 1) + ")")
		},
		...
		getSelectedRowIndex : function(options) {
			return $.fn.bsgrid.getRows(options).index(
					$.fn.bsgrid.getSelectedRow(options))
		},
		selectRow : function(row, options) {
			...
			if (!!options.settings.event.selectRowEvent) {
				options.settings.event.selectRowEvent($.fn.bsgrid
						.getRowRecord(trObj), row, trObj, options)
			}
		},
		unSelectRow : function(options) {
			...
			if (row != -1) {
				...
				if (!!options.settings.event.unselectRowEvent) {
					options.settings.event.unselectRowEvent($.fn.bsgrid
							.getRowRecord(trObj), row, trObj, options)
				}
			}
		},
					...
					.each(
							function(i) {
								if (columnsModel[i].sortName != "") {
									...
									var sortHtml = '<a href="javascript:void(0);" sortName="'
											+ sortName + '" class="sort ';
									if (sortOrder != ""
		...
		setGridBlankBody : function(options) {
			...
			if (curPageRowsNum != 0) {
				if (options.settings.stripeRows) {
					$.fn.bsgrid.getRows(options).filter(":even").addClass(
							"even_index_row")
				}
		...
		setPagingValues : function(options) {
			$.fn.bsgrid.getPagingObj(options).setPagingValues(options.curPage,
					options.totalRows)
		}
	}
})(jQuery);
(function($) {
	$.fn.bsgrid.defaults.extend.settings = {
		...
		gridEditConfigs : {
			text : {
				build : function(edit, value, record, rowIndex, colIndex,
						tdObj, trObj, options) {
					return value + '<input class="bsgrid_editgrid_edit" type="'
							+ edit + '" value="' + value + '"/>'
				},
			...
			checkbox : {
				build : function(edit, value, record, rowIndex, colIndex,
						tdObj, trObj, options) {
					return value
							+ '<input class="bsgrid_editgrid_checkbox" type="'
							+ edit + '" value="' + value + '"/>'
				},
				val : function(formObj) {
					return formObj.val()
			...
			textarea : {
				build : function(edit, value, record, rowIndex, colIndex,
						tdObj, trObj, options) {
					return value + '<textarea class="bsgrid_editgrid_edit">'
							+ value + "</textarea>"
				},
				val : function(formObj) {
					return formObj.val()
	...
	$.fn.bsgrid.extendInitGrid.initGridExtendOptions = function(gridId, options) {
		var columnsModel = options.columnsModel;
		var colsProperties = options.settings.colsProperties;
		$.fn.bsgrid.getGridHeaderObject(options).each(
				function(i) {
					columnsModel[i].lineNumber = $.trim($(this).attr(
							colsProperties.lineNumberAttr));
					columnsModel[i].check = $.trim($(this).attr(
							colsProperties.checkAttr));
					columnsModel[i].edit = $.trim($(this).attr(
							colsProperties.editAttr))
				});
		if ($("#" + options.gridId + " tfoot tr td[" + colsProperties.aggAttr
				+ "!='']").length != 0) {
			$("#" + options.gridId + " tfoot tr td")
					.each(
							function(i) {
								columnsModel[i].aggName = "";
								columnsModel[i].aggIndex = "";
								var aggInfo = $.trim($(this).attr(
										colsProperties.aggAttr));
								if (aggInfo.length != 0) {
									var aggInfoArray = aggInfo.split(",");
									columnsModel[i].aggName = aggInfoArray[0]
											.toLocaleLowerCase();
		...
		if (options.settings.extend.settings.supportGridEdit) {
			...
			gridObj.getChangedRowsIndexs = function() {
				return $.fn.bsgrid.defaults.extend
						.getChangedRowsIndexs(options)
			};
			gridObj.getChangedRowsOldRecords = function() {
				return $.fn.bsgrid.defaults.extend
						.getChangedRowsOldRecords(options)
			};
			gridObj.getRowsChangedColumnsValue = function() {
				return $.fn.bsgrid.defaults.extend
						.getRowsChangedColumnsValue(options)
			};
		...
		if (options.settings.extend.settings.fixedGridHeader) {
			...
			options.settings.extend.afterRenderGridMethods.fixedHeader = function(
					parseSuccess, gridData, options) {
				$.fn.bsgrid.extendOtherMethods.fixedHeader(false, options)
			}
		}
		if ($
				.trim(options.settings.extend.settings.searchConditionsContainerId) != "") {
			options.settings.extend.initGridMethods.initSearchConditions = $.fn.bsgrid.extendInitGrid.initSearchConditions
		}
		if ($("#" + options.gridId + " tfoot td[" + colsProperties.aggAttr
				+ "!='']").length != 0) {
	...
	$.fn.bsgrid.extendInitGrid.initSearchConditions = function(gridId, options) {
		...
		for ( var key in params) {
			conditionsHtml.append('<option value="' + key + '">' + params[key]
					+ "</option>")
		}
		conditionsHtml.append("</select>");
		conditionsHtml.append("&nbsp;");
		conditionsHtml
				.append('<input type="text" class="bsgrid_conditions_input" />');
		$("#" + options.settings.extend.settings.searchConditionsContainerId)
				.html(conditionsHtml.toString());
		$(
				"#"
						+ options.settings.extend.settings.searchConditionsContainerId
						+ " select.bsgrid_conditions_select").change(
				function() {
					$(this).next("input.bsgrid_conditions_input").attr("name",
							$(this).val())
				}).trigger("change")
	};
					...
					$(obj)
							.mousemove(
									function(e) {
										...
										if ((i != 0 && e.clientX - left < 10)
												|| (i != headLen - 1 && e.clientX
														- nLeft > -10)) {
											$(obj).css({
												cursor : "e-resize"
											});
											if ($.trim($(obj).data(
													"ex_mousedown")) != "mousedown") {
												return
											}
											var mWidth = $(mObj).width();
											var newMWidth = mWidth - e.clientX
													+ $(mObj).offset().left;
											var preMWidth = $(mObj).prev()
													.width();
											var preNewMWidth = preMWidth
													+ e.clientX
													- $(mObj).offset().left;
											if (parseInt(newMWidth) > 19
													&& parseInt(preNewMWidth) > 19) {
												$(mObj).width(newMWidth).prev()
														.width(preNewMWidth)
											}
										} else {
											$(mObj).css({
	...
	$.fn.bsgrid.extendRenderPerColumn.renderCheck = function(record, rowIndex,
		...
		if (rowIndex < options.curPageRowsNum) {
			...
			if (columnModel.check == "true") {
				return '<input class="bsgrid_editgrid_check" type="checkbox" value="'
						+ $.fn.bsgrid.getRecordIndexValue(record,
								columnModel.index, options) + '"/>'
			}
		}
		return false
	};
	$.fn.bsgrid.extendRenderPerColumn.renderForm = function(record, rowIndex,
			colIndex, tdObj, trObj, options) {
		if (rowIndex < options.curPageRowsNum) {
			var columnModel = options.columnsModel[colIndex];
			var edit = columnModel.edit;
			var value = $.fn.bsgrid.getRecordIndexValue(record,
					columnModel.index, options);
			var tdHtml = "&nbsp;";
			if (edit in options.settings.extend.settings.gridEditConfigs) {
				tdHtml = options.settings.extend.settings.gridEditConfigs[edit]
						.build(edit, value, record, rowIndex, colIndex, tdObj,
								trObj, options)
			} else {
				return false
			}
								...
								function() {
									...
									formObj
											.bind(
													...
													function(event) {
														options.settings.event.customCellEditEvents[key]
																(
																		event.data.formObj,
																		event.data.record,
																		event.data.rowIndex,
	...
	$.fn.bsgrid.extendAfterRenderGrid.renderLineNumber = function(parseSuccess,
			gridData, options) {
		$.fn.bsgrid.getGridHeaderObject(options).each(
				function(i) {
					...
					if (num == "line" || num == "total_line") {
						$.fn.bsgrid.getRows(options).filter(
								":lt(" + options.curPageRowsNum + ")").find(
	...
	$.fn.bsgrid.extendAfterRenderGrid.addCheckChangedEvent = function(
			parseSuccess, gridData, options) {
		$.fn.bsgrid
				.getGridHeaderObject(options)
				.each(
						function(hi) {
							if (options.columnsModel[hi].check == "true") {
								var checkboxObj = $(this).find(
										"input[type=checkbox]");
								var checkboxObjs = $.fn.bsgrid
										.getRows(options)
										.find(
												"td:nth-child("
														+ (hi + 1)
								...
								checkboxObjs
										.change(function() {
											...
											if (!checked
													&& allCheckboxObjs
															.filter(":checked").length == allCheckboxObjs.length) {
												$.bsgrid.adaptAttrOrProp(
														checkboxObj, "checked",
														true)
											} else {
												if (checked
	...
	$.fn.bsgrid.extendAfterRenderGrid.addGridEditEvent = function(parseSuccess,
			gridData, options) {
		var gridObj = $.fn.bsgrid.getGridObj(options.gridId);
		$.fn.bsgrid
				.getRows(options)
				.filter(":lt(" + options.curPageRowsNum + ")")
				.each(
						function() {
							var columnsModel = options.columnsModel;
							$(this)
															...
															.change(
																	function() {
																		...
																		var isNew = $
																				.trim(rowObj
																						.data("new"));
																		var value = (isNew == "true" ? ""
																				: gridObj
																						.getRecordIndexValue(
																								gridObj
																										.getRowRecord(rowObj),
																								columnsModel[ci].index));
																		if ($
																				.trim($(
																						this)
																						.val()) != value) {
																		...
																		rowObj
																				.data(
																						"change",
																						rowObj
																								.find(".bsgrid_editgrid_change").length)
																	})
												}
											});
							if (options.settings.extend.settings.supportGridEditTriggerEvent == "") {
								$(this).find(".bsgrid_editgrid_hidden").each(
										function() {
											showCellEdit(this)
										})
							} else {
		...
		function showCellEdit(formObj) {
			var cloneObj = $(formObj).removeClass("bsgrid_editgrid_hidden")
					.clone(true);
			$(formObj).parent("td").html(cloneObj)
		}
	};
	$.fn.bsgrid.extendAfterRenderGrid.aggregation = function(parseSuccess,
			gridData, options) {
		var gridObj = $.fn.bsgrid.getGridObj(options.gridId);
		var columnsModel = options.columnsModel;
		$(
				"#" + options.gridId + " tfoot tr td["
						+ options.settings.colsProperties.aggAttr + "!='']")
				.each(
						function(i) {
							if (columnsModel[i].aggName != "") {
								...
								if (aggName == "count") {
									...
									if (aggName == "countnotnone"
																...
																if (aggName == "countnotnone") {
																	...
																	if (aggName == "sum"
																		...
																		if (aggName == "max"
																			...
																			if (!isNaN(rval)
																					&& (val == null
																							|| (aggName == "max" && parseFloat(rval) > val) || (aggName == "min" && parseFloat(rval) < val))) {
																				val = parseFloat(rval)
																			}
																		} else {
																			if (aggName == "concat") {
										...
										if (aggName == "custom") {
											val = eval(columnsModel[i].aggIndex)
													(gridObj, options)
										}
									}
								}
	...
	$.fn.bsgrid.defaults.extend.getCheckedRowsRecords = function(options) {
		...
		$.each($.fn.bsgrid.defaults.extend.getCheckedRowsIndexs(options),
				function(i, rowIndex) {
					records[records.length] = $.fn.bsgrid.getRecord(rowIndex,
							options)
				});
		return records
	};
	$.fn.bsgrid.defaults.extend.getCheckedValues = function(index, options) {
		...
		$.each($.fn.bsgrid.defaults.extend.getCheckedRowsRecords(options),
				function(i, record) {
					values[values.length] = $.fn.bsgrid.getRecordIndexValue(
	...
	$.fn.bsgrid.defaults.extend.activeGridEditMode = function(options) {
		...
		$.fn.bsgrid.getRows(options).filter(
				":lt(" + options.curPageRowsNum + ")").find(
				"td .bsgrid_editgrid_hidden").each(
				function() {
					var cloneObj = $(this)
							.removeClass("bsgrid_editgrid_hidden").clone(true);
									...
									.each(
											function(ci) {
												if ($(this)
													...
													values["row_" + rowIndex][options.columnsModel[ci].index] = options.settings.extend.settings.gridEditConfigs[options.columnsModel[ci].edit]
															.val($(this)
																	.find(
																			".bsgrid_editgrid_change"))
												}
	...
	$.fn.bsgrid.defaults.extend.addNewEditRow = function(options) {
		...
		$("#" + options.gridId + " tbody").prepend(
				gridObj.getRow(0).clone(true));
		gridObj.getRowCells(0).each(
				function(colIndex) {
					var columnModel = options.columnsModel[colIndex];
					if (columnModel.render != "") {
						var render_method = eval(columnModel.render);
						var render_html = render_method(null, 0, colIndex,
								options);
						...
						$(this).html(
								$(this).children().removeClass(
										"bsgrid_editgrid_change").clone(true))
								.removeAttr("title")
					}
				});
	...
	$.fn.bsgrid.extendOtherMethods.fixedHeader = function(iFirst, options) {
		...
		if (!iFirst) {
			...
			$("#" + options.gridId + " thead tr:lt(" + headTrNum + ")")
					.remove()
		}
		...
		if (fixedGridHeight < $("#" + options.gridId).height()) {
			...
			$("#" + options.gridId).width(
					$("#" + options.gridId + "_fixedDiv").width() - 18);
			$("#" + options.gridId + "_fixedDiv").animate({
				scrollTop : "0px"
			}, 0)
		} else {
			$("#" + options.gridId + "_fixedDiv").height(
					$("#" + options.gridId).height());
			$("#" + options.gridId).width(
					$("#" + options.gridId + "_fixedDiv").width() - 1)
		}
		$("#" + options.gridId + " thead tr:lt(" + headTrNum + ")").clone(true)
				.prependTo("#" + options.gridId + " thead");
		$("#" + options.gridId + " thead tr:lt(" + headTrNum + ")").css({
			"z-index" : 10,
			position : "fixed"
		}).width($("#" + options.gridId + " thead tr:last").width());
		$("#" + options.gridId + " thead tr:lt(" + headTrNum + ")").each(
				function(i) {
					var position = $(
							"#" + options.gridId + " thead tr:eq("
									+ (headTrNum + i) + ")").position();
					$(this).css(
							{
								top : position.top
										- getSize($(this).find("th").css(
												"border-top-width")),
								left : position.left
							})
				});
		$("#" + options.gridId + " thead tr:gt(" + (headTrNum - 1) + ")")
				.each(
						function(ri) {
							$(this)
									.find("th")
		...
		doExport : function(h, o, d) {
			...
			for ( var f = 0; f < h.length; f++) {
				if (a.trim(h.eq(f).attr(c.colsProperties.exportAttr)) != "false") {
					...
					g = g + ","
							+ a.trim(h.eq(f).attr(c.colsProperties.indexAttr));
					var l = a.trim(h.eq(f).attr(c.colsProperties.widthAttr))
							.toLocaleLowerCase();
			...
			document.location.href = c.url
					+ (c.url.indexOf("?") < 0 ? "?" : "&")
					+ c.requestParamsName.exportFileName + "="
					+ encodeURIComponent(encodeURIComponent(c.exportFileName))
					+ "&" + c.requestParamsName.colNames + "="
					+ encodeURIComponent(encodeURIComponent(n.substring(1)))
					+ "&" + c.requestParamsName.colIndexs + "="
					+ g.substring(1) + "&" + c.requestParamsName.colWidths
					+ "=" + m.substring(1) + "&"
					+ c.requestParamsName.colAligns + "=" + j.substring(1)
					+ (b.length == 0 ? "" : ("&" + b))
		}
	}
})(jQuery);
