...
$(function () {
	...
	$(".control .add").click(function() {
		...
		var editurl = "${pageContext.request.contextPath}/purchaseApplication/editEmp.do";
			$.ajax({
				type : "get",
				dataType : "json",
				url : editurl,
		...
		$.basewindow("新增办公用品采购", $chuangkou,1200, 400);
	...
	$(".control .mid").click(function(){
		...
		var id =  grid.getCheckedValues('id');
			if (id != "") {
			var editurl = "${pageContext.request.contextPath}/purchaseApplication/editOff.do";
			$.ajax({
				type : "get",
				dataType : "json",
				url : editurl,
			...
			$.basewindow("修改办公用品采购",$chuangkou, 1200, 300);
			$(".basewindow").addClass("custom-scroll");
	...
	$(".okok").click(function(){
	    ...
	    $.ajax({
	        type : "POST",
			url : "${pageContext.request.contextPath}/purchaseApplication/saveOff.do",
			cache: false,
	...
	$(".ok").click(function(){
	    ...
	    $.ajax({
	        type : "POST",
			url : "${pageContext.request.contextPath}/purchaseApplication/midOff.do",
			cache: false,
	...
	$(".del").click(function(){
	    var id =  grid.getCheckedValues('id');
			...
			var editurl = "${pageContext.request.contextPath}/purchaseApplication/deleteOff.do";
			$.ajax({
				type : "get",
				dataType : "json",
				url : editurl,
	...
	var grid = $.fn.bsgrid.init("searchTable", {
    	url: "${pageContext.request.contextPath}/purchaseApplication/readPagesR.do?branchs=${branchs}&purType=${purType}",
        pageSizeSelect: true,
        pageSize: pageSize,
...
function seanot(id){
		...
		function add() {  
		  ...
		  numbers.innerHTML = "<td><input type='text' name='numbers' id='numbers" + count + "' onkeyup='test("+count+")' onkeyup='value=value.replace(/[^1-9]/g,'')'   style='width:150px;'/></td>";  
		  var taxMoney = tr.insertCell(4);  
		  taxMoney.innerHTML = "<td><input type='text' name='taxMoney' id='taxMoney" + count + "'   onkeyup='test("+count+")' onchange='this.value=(this.value.match(/\d+(\.\d{0,2})?/)||[''])[0]' style='width:150px;'/></td>";  
...
<div class="rightPadd">
	<div class="right-twohalf">
		<span>首页</span> <span>></span> <span>行政系统</span> <span>></span> <span class="color99">采购申请</span>
	</div>
			...
			<table class="table">
					...
					<table style="width:100%"> 
					  ...
					  <tbody>  
			                ...
			                <td class="taL"><input type="text" name="mtaxMoney" id="mtaxMoney"   onkeyup="midf()" onchange="this.value=(this.value.match(/\d+(\.\d{0,2})?/)||[''])[0]" style="width:150px;"/></td>
